...
void CReplayFrame::CreateReplayFrame(void){
  ...
  assert(SessionCounter() != nullptr);
	CString path = ReplayHTMLFilename(SessionCounter()->session_id(), _next_replay_frame);
...
CString CReplayFrame::GeneralInfo() {
  ...
  result += "<tr><td>\n";
  result += "Session: ";
	result += Number2CString(SessionCounter()->session_id(), 0);
  result += "</td></tr>\n";
  // Frame number 
  result += "<tr><td>\n";
  CString next_frame;
...
CString CReplayFrame::GetPlayerInfoAsHTML() {
	...
	for (int i=0; i<BasicScraper()->Tablemap()->nchairs(); i++) {
		...
		// seated, active, button, dealt, playing
		text = "      <td>";
		text +=	(EngineContainer()->symbol_engine_active_dealt_playing()->playersseatedbits() & (1<<i)) ? "s" : "-";
		text +=	(EngineContainer()->symbol_engine_active_dealt_playing()->playersactivebits() & (1<<i)) ? "a" : "-";
		text +=	(DEALER_CHAIR == i) ? "b" : "-";
		text +=	(EngineContainer()->symbol_engine_active_dealt_playing()->playersdealtbits() & (1<<i)) ? "d" : "-";
		text +=	(EngineContainer()->symbol_engine_active_dealt_playing()->playersplayingbits() & (1<<i)) ? "p" : "-";
		text += "</td>\n";
		player_info += text;  
		// Cards
    text.Format("      <td>%s</td>\n",
...
CString CReplayFrame::GetButtonStatesAsHTML() {
  ...
  // Table header for: FCKRA
	...
	button_states += "  <tr>\n";
	// Buttons
	CString fckra_seen = EngineContainer()->symbol_engine_autoplayer()->GetFCKRAString();
	// Table footer
	button_states += "    <td>";
...
CString CReplayFrame::GetBlindInfoAsHTML() {
	...
	text.Format("  <tr><th>sb</th><td>%11.2f</td></tr>\n", EngineContainer()->symbol_engine_tablelimits()->sblind());
	blind_info += text;
...
void CReplayFrame::CreateReplaySessionDirectoryIfNecessary() {
  assert(SessionCounter() != nullptr);
	CString path = ReplaySessionDirectory(SessionCounter()->session_id());
...
void CReplayFrame::CreateBitMapFile() {
  ...
  const char *szFile = ReplayBitmapFilename(SessionCounter()->session_id(), _next_replay_frame);
