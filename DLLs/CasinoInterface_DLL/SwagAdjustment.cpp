...
double RoundToBeautifulBetsize(const double amount_to_raise_to) {
  assert(amount_to_raise_to >= EngineContainer()->symbol_engine_tablelimits()->bblind());
  ...
  if (amount_to_raise_to < (2.6 * EngineContainer()->symbol_engine_tablelimits()->bblind())) {
    write_log(Preferences()->debug_betsize_adjustment(),
  ...
  } else if (amount_to_raise_to == Rounding(amount_to_raise_to, EngineContainer()->symbol_engine_tablelimits()->bblind())) {
...
double MaximumBetsizeDueToMaxOppStack() {
	double maximum = EngineContainer()->symbol_engine_chip_amounts()->call() + TableState()->User()->_bet.GetValue() + EngineContainer()->symbol_engine_chip_amounts()->MaxActiveOpponentStack();
